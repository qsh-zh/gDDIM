...
def get_dataset(config, additional_dim=None, uniform_dequantization=False, evaluation=False):
  ...
  if "ps" in config.data.dataset:
      return get_ps_dataset(batch_dims, evaluation=evaluation)

  # Create dataset builders for each dataset.
  if config.data.dataset == 'CIFAR10':
    dataset_builder = tfds.builder('cifar10')
    train_split_name = 'train'
    if config.data.is_partial:
        train_split_name = tfds.core.ReadInstruction(
            'train',
            from_=0,
            to=0.1,
            unit='%'
  ...
  def create_dataset(dataset_builder, split):
    ...
    if isinstance(dataset_builder, tfds.core.DatasetBuilder):
      ...
      ds = dataset_builder.as_dataset(
        split=split, shuffle_files=not config.data.is_partial, read_config=read_config)
    else:
      ds = dataset_builder.with_options(dataset_options)
    ds = ds.repeat(count=num_epochs)
    if not config.data.is_partial:
...
def circle_generate_sample(N, noise=0.25):
    ...
    pos = np.stack([np.cos(angle), np.sin(angle)])
...
def get_ps_dataset(batch_dims,evaluation=False):
    num_epochs = None if not evaluation else 1
    def create_dataset():
        raw_data = olympic_generate_sample(128 * 100, noise=0.01)
        mean_data = np.mean(raw_data, axis=0, keepdims=True)
        std_data = np.std(raw_data, axis=0, keepdims=True)
        proc_data = (raw_data - mean_data) / std_data
        np.random.shuffle(proc_data)
        
        prefetch_size = tf.data.experimental.AUTOTUNE
        shuffle_buffer_size = 128 * 8 * 5 # used for trainset shuffle

        raw_dataset = tf.data.Dataset.from_tensor_slices({"image":proc_data})
        ...
        for batch_size in reversed(batch_dims):
            ds = ds.batch(batch_size, drop_remainder=True)
        return ds.prefetch(prefetch_size)
    return create_dataset(), create_dataset(), None
