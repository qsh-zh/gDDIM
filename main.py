...
flags.DEFINE_enum("mode", None, ["train", "eval", "sampling", "fid", "check", "fid_stats"], "Running mode: train or eval")
...
def resolve_result_folder(config, ckpt_path):
  ...
  if sampler_name.lower() == 'sdeis':
    if not config.sampling.sdeis_use_order0 or config.sampling.deis_order>0:
      info_path=f"sdeis_lambda{config.sampling.lambda_coef}_order{config.sampling.deis_order}_nfe{config.sampling.nfe}_ts{config.sampling.ts_order}{'denoising' if config.sampling.noise_removal else ''}"
    else:
      info_path=f"sdeis_order0_lambda{config.sampling.lambda_coef}_order{config.sampling.deis_order}_nfe{config.sampling.nfe}_ts{config.sampling.ts_order}{'denoising' if config.sampling.noise_removal else ''}"
    return os.path.join(f"{ckpt_path}_eval", info_path)
  elif sampler_name.lower() == 'ldeis':
    info_path=f"Ldeis_order{config.sampling.deis_order}_nfe{config.sampling.nfe}_ts{config.sampling.ts_order}{'denoising' if config.sampling.noise_removal else ''}"
    return os.path.join(f"{ckpt_path}_eval", info_path)
