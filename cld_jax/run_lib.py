...
#     writer = tensorboard.SummaryWriter(tb_dir)
  ...
  initial_step = int(state.step // config.training.log_freq) * config.training.log_freq
  ...
  for step in range(initial_step, num_train_steps + 1, config.training.n_jitted_steps):
    ...
    if step != initial_step and step % config.training.snapshot_freq_for_preemption == 0 and jax.host_id() == 0:
    ...
    if step !=initial_step and step % config.training.ema_update_freq == 0 and jax.host_id() == 0:
      state = flax_utils.unreplicate(pstate)
      # occasionally upadte model params enable fast convergence
      optimizer = losses.get_optimizer(config).create(state.params_ema)
      state = state.replace(optimizer=optimizer)
      pstate = flax_utils.replicate(state)
      logging.info(f"{'Train step':>10}: {step:>9}" + '\t' + "update from ema")
    ...
    if step % config.training.snapshot_freq_for_sampling == 0 or step == num_train_steps:
      if config.training.snapshot_sampling:
        ...
        sample, _, n = sampling_fn(sample_rng, pstate, config.training.snapshot_sampling_batch // jax.local_device_count())
  ...
  for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
    ...
    if config.eval.enable_loss:
      ...
      for i, batch in enumerate(eval_iter):
        ...
        eval_loss = flax_utils.unreplicate(p_eval_loss)
...
def sample_data(config,
  ...
  sde = sde_lib.from_config(config)
  if jax.host_id() == 0:
    logging.critical("Initialized sde and state")
  ...
  for r in tqdm.tqdm(range(0, num_sampling_rounds)):
    rng, *sample_rng = jax.random.split(rng, jax.local_device_count() + 1)
    f_sample = os.path.join(result_folder, f"samples_{r}.npz")
    if os.path.exists(f_sample) and is_continue:
      if jax.host_id() == 0:
        logging.critical(f"SKIP!!! Already exists {f_sample}")
      continue
    sample_rng = jnp.asarray(sample_rng)
    samples_org_x, samples_v, nfe_cnt = sampling_fn(sample_rng, pstate, config.eval.batch_size // jax.local_device_count())
    samples_x = np.clip(samples_org_x * 255., 0, 255).astype(np.uint8)
    samples_x = samples_x.reshape(
      (-1, config.data.image_size, config.data.image_size, config.data.num_channels))
        # Write samples to disk or Google Cloud Storage
    with tf.io.gfile.GFile(f_sample, "wb") as fout:
      io_buffer = io.BytesIO()
      np.savez_compressed(io_buffer, samples=samples_x, nfe_cnt=nfe_cnt, samples_v=samples_v, samples_x=samples_org_x)
      fout.write(io_buffer.getvalue())
    gc.collect()
